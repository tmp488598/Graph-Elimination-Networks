...
class GENsConv(MessagePassing):
    ...
    def forward(self, x: Tensor, edge_index: Adj, x0: OptTensor = None,
        ...
        if self.hop_att:
            K, H, C = self.K, self.heads, self.out_channels
            x_q, x_k = preds[1:], preds[0]

            q = self.q(x_q).view(K, -1, H, C)  # Multi-head self-attention
            k = torch.unsqueeze(self.k(x_k).view(-1, H, C), dim=-2)
            att = torch.einsum('nxhd,xhyd->xnhy', [q, k])
            att = F.softmax(att, dim=1)
            preds = self.lin(x_q).view(K, -1, H, C).transpose(0, 1) * att
