...
from graphgps.loader.dataset.ZINC_no_cycle import ZINCNoCycle

from torch_geometric.transforms import Compose
from graphgps.loader.dataset.transforms import RandomWalkSampler,Preprocessor
...
def preformat_ZINC(dataset_dir, name):
    ...
    Returns:
        PyG dataset object
    """
    if name not in ['subset', 'full', 'nocycle']:
        raise ValueError(f"Unexpected subset choice for ZINC dataset: {name}")
    if name=='nocycle':
        dataset = join_dataset_splits(
            [ZINCNoCycle(root=dataset_dir, split=split)
             for split in ['train', 'val', 'test']])
    else:
        dataset = join_dataset_splits(
...
def preformat_VOCSuperpixels(dataset_dir, name, slic_compactness):
    ...
    Returns:
        PyG dataset object
    """
    transform = RandomWalkSampler(80, 0.5, window_size=16)
    test_transform = RandomWalkSampler(80, sample_rate=1.0, window_size=16)
    dataset = []
    for split in ['train', 'val', 'test']:
        if split=='train':
            dataset.append(VOCSuperpixels(root=dataset_dir, name=name,
                        slic_compactness=slic_compactness,
                        split=split, transform=transform))
        else:
            dataset.append(VOCSuperpixels(root=dataset_dir, name=name,
                                          slic_compactness=slic_compactness,
                                          split=split, transform=test_transform))
    dataset = join_dataset_splits(dataset)
...
def preformat_COCOSuperpixels(dataset_dir, name, slic_compactness):
    ...
    Returns:
        PyG dataset object
    """
    transform = RandomWalkSampler(100, 0.25, window_size=16)
    test_transform = RandomWalkSampler(100, sample_rate=1.0, window_size=16)
    dataset = []
    for split in ['train', 'val', 'test']:
        if split == 'train':
            dataset.append(COCOSuperpixels(root=dataset_dir, name=name,
                                          slic_compactness=slic_compactness,
                                          split=split, transform=transform))
        else:
            dataset.append(COCOSuperpixels(root=dataset_dir, name=name,
