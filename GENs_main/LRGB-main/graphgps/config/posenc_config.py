...
def set_cfg_posenc(cfg):
    ...
    for name in ['posenc_LapPE', 'posenc_SignNet',
                 'posenc_RWSE', 'posenc_RWSEEdge', 'posenc_HKdiagSE', 'posenc_ElstaticSE',
    ...
    for name in ['posenc_RWSE', 'posenc_RWSEEdge', 'posenc_HKdiagSE', 'posenc_ElstaticSE']:
        pecfg = getattr(cfg, name)
    ...
    cfg.posenc_RWSEEdge.num_global = 2
    cfg.posenc_RWSEEdge.global_edge_dropout = 0.2
