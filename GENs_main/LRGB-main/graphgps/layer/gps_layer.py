...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        elif local_gnn_type == "GENsConv":
            self.local_model = GENConvLayer(dim_h, dim_h,
                                             dropout=dropout,
                                             residual=False  # True,
            )
    ...
    def forward(self, batch):
        ...
        if self.local_model is not None:
            ...
            if self.local_gnn_type == 'CustomGatedGCN':
            ...
            elif self.local_gnn_type == 'GENsConv':
                es_data = None
                if self.equivstable_pe:
                    es_data = batch.pe_EquivStableLapPE
                local_out = self.local_model(Batch(batch=batch,
                ...
                h_local = h_local + h_in1 # Residual connection.
        ...
        if self.has_global:
            ...
            if self.global_model_type == 'Transformer':
                ...
                h_global = self.self_attn(h_dense)[mask]
                ...
                h_global = self.self_attn(h_dense)[mask][h_ind_perm_reverse]
                ...
                if batch.split == 'train':
                    ...
                    for i in range(5):
                        ...
                        h_ind_perm_reverse = torch.argsort(h_ind_perm)
                        h_global = self.self_attn(h_dense)[mask][h_ind_perm_reverse]
                        mamba_arr.append(h_global)
                    h_global = sum(mamba_arr) / 5
                ...
                if batch.split == 'train':
                    ...
                    if self.global_model_type.split('_')[-1] == 'Multi':
                        ...
                        for mod in self.self_attn:
                            ...
                            h_global = mod(h_dense)[mask][h_ind_perm_reverse]
                            h_global_list.append(h_global)
                        h_global = sum(h_global_list) / len(h_global_list)
                    else:
                        h_global = self.self_attn(h_dense)[mask][h_ind_perm_reverse]
                else:
                    mamba_arr = []
                ...
                if batch.split == 'train':
                    ...
                    h_global = torch.cat(emb_arr)[h_ind_perm_reverse]
                else:
                    mamba_arr = []
                ...
                if batch.split == 'train':
                    ...
                    for i in range(5):
                        unique_cluster_n = len(torch.unique(batch.LouvainCluster))
                        permuted_louvain = torch.zeros(batch.LouvainCluster.shape).long().to(
                            batch.LouvainCluster.device)
                        random_permute = torch.randperm(unique_cluster_n + 1).long().to(batch.LouvainCluster.device)
                ...
                h_global = self.self_attn(h_dense)[mask][aug_idx_reverse]
            else:
