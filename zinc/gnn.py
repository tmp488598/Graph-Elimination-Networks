...
class GNN(torch.nn.Module):
    def __init__(self, args):
        ...
        #     nn_layer = MLP(hidden_channels,hidden_channels,hidden_channels, 2, self.dropout)
        ...
        for _ in range(num_layers):
            ...
            # self.convs.append(GNNConv(hidden_channels, hidden_channels))
            self.bns.append(torch.nn.BatchNorm1d(hidden_channels))
...
def main():
    ...
    #gens [0.1054,0.0835,0.0860,0.0950,0.1059]
    #GAT 0.1513 ± 0.0162
    #GCN 0.1413 ± 0.0117
    #GIN 0.1966 ± 0.0142
    ...
    _, eval_no_cycle_list = split_dataset_by_cycle(eval_dataset)
    #
    print(len(train_no_cycle_list))
    print(len(test_no_cycle_list))
    print(len(eval_no_cycle_list))
    #
    dataset = MyListDataset(train_no_cycle_list)
    test_dataset = MyListDataset(test_no_cycle_list)
    eval_dataset = MyListDataset(eval_no_cycle_list)

    train_sampler = DistributedSampler(dataset)
    # test_sampler = DistributedSampler(test_dataset, shuffle=False)

    train_loader = DataLoader(dataset, batch_size=64, sampler=train_sampler, num_workers=24)
    test_loader = DataLoader(test_dataset, batch_size=64,  shuffle=False, num_workers=24)
    eval_loader = DataLoader(eval_dataset, batch_size=64, shuffle=False, num_workers=24)
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, args.epoch):
            ...
            if local_rank==0:
                ...
                eval_loss = test(model, local_rank, eval_loader, criterion)
                logger.add_result(run, [train_loss, test_loss, eval_loss])
                if epoch%args.log_steps==0:
                    print(f'Run: {run + 1:02d}, Epoch: {epoch:02d}, Train Loss: {train_loss:.4f}, Eval Loss: {eval_loss:.4f}, Test loss:{test_loss:.4f}')
        if local_rank == 0:
...
class Logger(object):
    ...
    def print_statistics(self, run=None):
        ...
        if run is not None:
            result = torch.tensor(self.results[run])  # [epochs, 3]
            best_val_epoch = result[:, 1].argmin().item()

            best_train = result[best_val_epoch, 0].item()
            best_val = result[best_val_epoch, 1].item()
            best_test = result[best_val_epoch, 2].item()

            print(f'Run {run + 1:02d}:')
            print(f'Best Val loss: {best_val:.4f}')
            print(f'Corresponding Test loss: {best_test:.4f}')
            print(f'(Train loss at this epoch: {best_train:.4f})')

        else:
            result = torch.tensor(self.results)  # [runs, epochs, 3]

            best_results = []
            for r in result:
                best_val_epoch = r[:, 1].argmin().item()
                best_val = r[best_val_epoch, 1].item()
                best_test = r[best_val_epoch, 2].item()
                best_results.append((best_val, best_test))

            best_results = torch.tensor(best_results)  # [runs, 2]

            print('All runs:')
            val_all = best_results[:, 0]
            test_all = best_results[:, 1]

            print(f'Best Val loss: {val_all.mean():.4f} ± {val_all.std():.4f}')
            print(f'Corresponding Test loss: {test_all.mean():.4f} ± {test_all.std():.4f}')
...
def split_dataset_by_cycle(dataset):

    dataset_with_cycle = []
    dataset_without_cycle = []

    for data in dataset:
        G = to_networkx(data, to_undirected=True, remove_self_loops=True)
        cycles = nx.cycle_basis(G)
        if len(cycles) > 0:
            dataset_with_cycle.append(data)
        else:
            dataset_without_cycle.append(data)

    return dataset_with_cycle, dataset_without_cycle
