...
#     def __init__(self, embed_size, ff_hidden, dropout = 0.0):
...
#
#     def forward(self, x):
#         x = self.ff_dropout1(F.relu(self.fc1(x)))
#         return self.ff_dropout2(self.fc2(x))

class FeedForward(torch.nn.Module):
    def __init__(self, embed_size, ff_hidden, dropout = 0.0, num_layers=2, bn_bool = True):
        super(FeedForward, self).__init__()
        ...
        for _ in range(num_layers - 1):
            self.lins.append(torch.nn.Linear(embed_size, ff_hidden))
            self.bns.append(torch.nn.BatchNorm1d(ff_hidden))
            embed_size = ff_hidden
        self.lins.append(torch.nn.Linear(ff_hidden, ff_hidden))
...
class GENsConv(MessagePassing):
    ...
    def __init__(self, in_channels: int, out_channels: int,
                 ...
                 edge_dim=None, concat=False, dropout=0, use_ffN = False, **kwargs):
        ...
        # self.norm1 = nn.LayerNorm(self.heads * out_channels) if self.concat else nn.LayerNorm(out_channels)
        if self.use_ffN:
            self.norm = nn.LayerNorm(self.heads * out_channels) if self.concat else nn.LayerNorm(out_channels)
            self.feed_forward = FeedForward(self.heads * out_channels, self.heads * out_channels, dropout=dropout) if self.concat else FeedForward(out_channels, out_channels, dropout=dropout)
    ...
    def forward(self, x: Tensor, edge_index: Adj, edge_attr: OptTensor = None, x0: OptTensor = None,
                edge_weight: OptTensor = None, deg=None) -> Tensor:
        """"""
        ...
        if self.hop_att:
            ...
            out = self.lin(preds.sum(0)) + x_t#或者吧xt的线性层再改一下
