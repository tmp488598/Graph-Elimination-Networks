...
class GNN_node(torch.nn.Module):
    ...
    def forward(self, batched_data):
        ...
        x = self.atom_encoder(x) + scatter(self.bond_encoder(edge_attr), edge_index[0], dim=0, dim_size=x.size(0), reduce='add')
...
#             elif gnn_type == 'gcn':
#                 self.convs.append(GCN2Conv(emb_dim))
#             else:
...
#                                     torch.nn.ReLU(), torch.nn.Linear(2 * emb_dim, emb_dim), torch.nn.BatchNorm1d(emb_dim),
#                                     torch.nn.ReLU()))
