...
def get_model_with_default_configs(model_name,
    ...
    if model_name == 'GNN':
        def foo(dataset):
            return GNN(dataset=dataset, hidden=hidden, num_conv_layers=num_conv_layers,
...
def run_experiment_finetune(result_PATH, result_feat,
                            dataset_feat_net_triple=create_n_filter_triple(args.dataset, 'deg+odeg100', 'GNN', gfn_add_ak3=True, reddit_odeg10=True, dd_odeg10_ak1=True),
                            get_model=get_model_with_default_configs):

    dataset_name, feat_str, net = dataset_feat_net_triple
    dataset = get_dataset(dataset_name, feat_str=feat_str, root=args.data_root)
    ...
    cross_validation_with_val_set(
                    ...
                    logger=logger,
                    n_splits=args.n_splits, result_PATH=result_PATH, result_feat=result_feat)
...
if __name__ == '__main__':
    ...
    result_feat = args.dataset + '_' + str(args.hidden) + '_' + str(args.dropout) + '_' + str(args.lr) + '_' + str(args.n_layers_conv) 
    print(result_PATH, result_feat)
    run_experiment_finetune(result_PATH, result_feat)
