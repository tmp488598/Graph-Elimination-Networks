...
from gens_conv import GENsConv
...
class MPNNs(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, local_layers=3, 
                 ...
                 ln=False, bn=False, jk=False, gnn='gcn', args=None):
        ...
        if not self.pre_linear:
            if gnn=='gat':
                ...
                self.local_convs.append(GENsConv(in_channels, hidden_channels, K=args.K,
                                                 gamma=args.gamma, base_model=args.base_model,
                                                 hop_att=args.hop_att, fea_drop=args.fea_drop))
            else:
        ...
        for _ in range(local_layers):
            if gnn=='gat':
                ...
                self.local_convs.append(GENsConv(hidden_channels, hidden_channels, K=args.K,
                                                 gamma=args.gamma, base_model=args.base_model,
                                                 hop_att=args.hop_att, fea_drop=args.fea_drop))
            else:
